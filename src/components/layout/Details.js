...
const Details = ({ title, titleLink, place, placeLink, time, location, description }) => {
  const ref = useRef(null)

  return (
    <li ref={ref} key={ref} className="px-8 lg:px-48 my-12">
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 text-lg my-4 dark:text-light md:justify-center">
          ...
          <span className="flex gap-x-2 items-center">
            ...
            <span className="whitespace-nowrap text-xs lg:text-lg">{location}</span>
          </span>
        </div>

        <p className="font-medium w-full text-sm lg:text-xl px-4 dark:text-light">{description}</p>
      </motion.div>
    </li>
  )
}
